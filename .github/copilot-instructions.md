...
the source and vendor hashes pinned in `packages.json`. The core set also carries `pause`,
the Linux-only C sandbox shim, built with `stdenv` rather than `buildGoModule`. (`gomod2nix`/`buildGoApplication` is used
